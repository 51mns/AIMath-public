...
def _validate_new_lock(
    ...
    *,
    ...
    ttl = (
        parse_time(bundle.payload["expires_at"])
        - parse_time(bundle.payload["acquired_at"])
    ).total_seconds() / 3600
    ...
    cap = base_state.portfolio.get("governance", {}).get(
        "default_actor_exclusive_lock_cap", 1
    )
    if task.get("parallelism") == "EXCLUSIVE":
        actor_active = 0
        for b in base_state.active_lock_bundles():
            atid = b.payload.get("task_id")
            if (
                b.payload.get("actor", {}).get("id") == expected_actor
                and base_state.tasks.get(atid, {}).get("parallelism") == "EXCLUSIVE"
            ):
                actor_active += 1
        if actor_active >= cap:
            errors.append(f"actor EXCLUSIVE lock cap {cap} already reached")
    return errors
...
def validate_lock_transition(
    ...
    *,
    ...
    if changed:
        ...
        extension = (
            parse_time(new.payload["expires_at"]) - parse_time(old.payload["expires_at"])
        ).total_seconds() / 3600
...
def validate_lock_only(
    ...
    try:
        ...
        base_errors = base_state.validate()
        if base_errors:
            return "INVALID", ["base Village state is invalid; lock operation cannot proceed"]
        head_state = VillageState(head_td.name).load()
        head_errors = head_state.validate()
        if head_errors:
...
def main() -> int:
    ...
    protected = (
        base_protected.get("protected_patterns", BOOTSTRAP_PROTECTED)
        if base_protected
        else BOOTSTRAP_PROTECTED
    )
    governance_only = (
        base_protected.get("governance_only_patterns", BOOTSTRAP_GOVERNANCE_ONLY)
        if base_protected
    ...
    if protected_changed and args.actor not in maintainers:
        errors.append(
            "non-maintainer changed protected governance path(s): "
    ...
    lock_operation = "NONE"
    if lock_only:
        lock_operation, lock_errors = validate_lock_only(
            args.base, args.head, args.actor, maintainers
        )
